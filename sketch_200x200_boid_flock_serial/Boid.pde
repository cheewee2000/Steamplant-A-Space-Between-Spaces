...
class Boid {
  ...
  void borders() {
    ...
    if (position.x < -r) position.x = width*gridSize/scale+r;
    if (position.y < -r) position.y = height*gridSize/scale+r;
    if (position.x > width*gridSize/scale+r) position.x = -r;
    if (position.y > height*gridSize/scale+r){
      //println("out");
      position.y = -r;
    }
  }
